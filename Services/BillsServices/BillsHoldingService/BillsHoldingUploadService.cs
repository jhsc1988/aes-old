...
{
    public class BillsHoldingUploadService : IBillsHoldingUploadService
    {
        ...
        public BillsHoldingUploadService(IWebHostEnvironment webHostEnvironment, IBillsHoldingTempCreateService billsHoldingTempCreateService,
                ...
                try
                {
                    ...
                    using (reader = new StreamReader(fileName))
                    {
                        ...
                        while (!reader.EndOfStream)
                        {
                            ...
                            {
                                if ((await _unitOfWork.BillsHolding.TempList(userId)).Count() >= 500)
                                {
                                ...
                                _ = await _billsHoldingTempCreateService.AddNewTemp(reader.ReadLine(), iznos.ToString(), null, userId);
